...
Em outras palavras: para cada primo `p` e cada par compatível `(i, j)`, pode-se transferir unidades de `p` de índices ímpares para índices pares, respeitando quantas vezes `p` divide cada valor. A resposta é o máximo total de transferências viáveis somado sobre todos os primos.
...
1. **`s → i`** (para todo índice ímpar `i` que contém `p`): capacidade = expoente de `p` em `a[i]`, ou seja, quantas vezes `p` divide `a[i]`. Representa o "estoque" disponível de `p` no lado ímpar.

2. **`i → j`** (para cada par `(i, j)` fornecido, onde `i` é ímpar e `j` é par, e ambos contêm `p`): capacidade = expoente de `p` em `a[i]`. Representa a permissão de transferir `p` daquele índice ímpar para o par.

3. **`j → t`** (para todo índice par `j` que contém `p`): capacidade = expoente de `p` em `a[j]`. Representa o "limite de absorção" do lado par.

### Origem e Sorvedouro

- **Origem `s`**: injeta o fluxo disponível de cada primo `p` nos índices ímpares. Uma unidade de fluxo representa uma ocorrência de `p` passível de ser "aproveitada".
- **Sorvedouro `t`**: coleta o fluxo absorvido pelos índices pares. O valor máximo de fluxo que chega em `t` representa o total de fatores `p` que podem ser efetivamente emparelhados entre um índice ímpar e um par via algum dos pares permitidos.

### Justificativa das Capacidades

- A aresta `s → i` com capacidade = expoente de `p` em `a[i]` garante que não se use mais do que o disponível no elemento ímpar.
- A aresta `j → t` com capacidade = expoente de `p` em `a[j]` garante que não se absorva mais do que o disponível no elemento par.
- A aresta `i → j` representa a permissão concedida pelo enunciado: só se pode emparelhar `i` e `j` se esse par foi explicitamente fornecido. A capacidade limitada pelo expoente em `i` evita que `a[i]` "forneça" mais do que tem.
...
Para cada primo `p`, o valor do fluxo máximo de `s` a `t` representa exatamente quantos fatores `p` podem ser "transferidos" de algum índice ímpar para algum índice par via pares permitidos. A soma de todos esses fluxos, sobre todos os primos, é a resposta do problema.
...
- **Primo presente apenas em índices do mesmo lado (só ímpares ou só pares):** não há aresta de `i` para `j` com aquele primo; fluxo máximo é zero.
- **Pares onde um dos índices não contém o primo `p`:** a aresta `i → j` não é criada para esse primo, pois não há transferência possível.
